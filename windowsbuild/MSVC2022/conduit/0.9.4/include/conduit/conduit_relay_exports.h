...
#if defined(CONDUIT_RELAY_EXPORTS) || \
    defined(conduit_relay_EXPORTS) || \
    defined(CONDUIT_RELAY_MPI_EXPORTS) || \
    defined(conduit_relay_mpi_EXPORTS) || \
    defined(CONDUIT_RELAY_MPI_IO_EXPORTS) || \
    defined(conduit_relay_mpi_io_EXPORTS)
    /* define catch all def */
    #define CONDUIT_RELAY_EXPORTS_DEFINED 1
#endif

#if defined(_WIN32)
    #if defined(CONDUIT_WINDOWS_DLL_EXPORTS)
        #if defined(CONDUIT_RELAY_EXPORTS_DEFINED)
            #define CONDUIT_RELAY_API __declspec(dllexport)
        #else
            #define CONDUIT_RELAY_API __declspec(dllimport)
        #endif
    #else
        #define CONDUIT_RELAY_API /* empty for static builds */
    #endif
...
#else
    #if __GNUC__ >= 4 && defined(CONDUIT_RELAY_EXPORTS_DEFINED)
