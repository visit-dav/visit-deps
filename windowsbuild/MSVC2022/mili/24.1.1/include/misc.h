...
#define RENEW_N(type, old, cnt, add, descr) \
    ((type *)my_realloc((void *)(old), (cnt) * sizeof(type), (add) * sizeof(type), descr))
#define RENEWC_N(type, old, cnt, add, descr) \
    ((type *)mili_recalloc((void *)(old), (cnt) * sizeof(type), (add) * sizeof(type), descr))
#endif

#ifdef DEBUG_MEM
char *my_calloc();
#define NEW(type, descr) ((type *)my_calloc(1, sizeof(type), descr))
#define NEW_N(type, cnt, descr) ((type *)my_calloc((cnt), sizeof(type), descr))
#define RENEW_N(type, old, cnt, add, descr) \
    ((type *)my_realloc((void *)(old), (cnt) * sizeof(type), (add) * sizeof(type), descr))
#define RENEWC_N(type, old, cnt, add, descr) \
    ((type *)mili_recalloc((void *)(old), (cnt) * sizeof(type), (add) * sizeof(type), descr))
