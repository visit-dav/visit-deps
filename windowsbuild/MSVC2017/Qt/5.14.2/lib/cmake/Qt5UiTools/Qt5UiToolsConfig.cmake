...
set(Qt5UiTools_VERSION_STRING 5.14.2)
...
function(_qt5_UiTools_process_prl_file prl_file_location Configuration lib_deps link_flags)
    set(_lib_deps)
    set(_link_flags)

    set(_qt5_install_libs "${_qt5UiTools_install_prefix}/lib/")
            ...
            elseif(_framework_flag OR _flag MATCHES "^-l(.*)$")
                ...
                elseif(_lib MATCHES "^pthread$")
                    ...
                    if(current_search_paths)
                        find_library(_Qt5UiTools_${Configuration}_${_lib}_PATH ${_lib} HINTS ${current_search_paths} NO_DEFAULT_PATH)
                    endif()
                    find_library(_Qt5UiTools_${Configuration}_${_lib}_PATH ${_lib} HINTS ${CMAKE_CXX_IMPLICIT_LINK_DIRECTORIES})
                    mark_as_advanced(_Qt5UiTools_${Configuration}_${_lib}_PATH)
                    if(_Qt5UiTools_${Configuration}_${_lib}_PATH)
                        list(APPEND _lib_deps
                            ${_Qt5UiTools_${Configuration}_${_lib}_PATH}
                        )
                    else()
...
macro(_populate_UiTools_target_properties Configuration LIB_LOCATION IMPLIB_LOCATION
    ...
    set(_deps
        ${_Qt5UiTools_LIB_DEPENDENCIES}
    )
    set(_static_deps
        ${_Qt5UiTools_STATIC_${Configuration}_LIB_DEPENDENCIES}
    )
    ...
    set_property(TARGET Qt5::UiTools APPEND PROPERTY INTERFACE_LINK_LIBRARIES
                 "${_deps}"
    )
    ...
    set(_static_link_flags "${_Qt5UiTools_STATIC_${Configuration}_LINK_FLAGS}")
...
# added by KSB to fix link error with VisIt
            string(REPLACE " " ";" _static_link_flags_genex "${_static_link_flags_genex}")
...
if (NOT TARGET Qt5::UiTools)
    ...
    set(Qt5UiTools_PRIVATE_INCLUDE_DIRS
        "${_qt5UiTools_install_prefix}/include/QtUiTools/5.14.2"
        "${_qt5UiTools_install_prefix}/include/QtUiTools/5.14.2/QtUiTools"
    )
    ...
    foreach(_module_dep ${_Qt5UiTools_MODULE_DEPENDENCIES})
        if (NOT Qt5${_module_dep}_FOUND)
            find_package(Qt5${_module_dep}
                5.14.2 ${_Qt5UiTools_FIND_VERSION_EXACT}
    ...
    if(NOT Qt5_EXCLUDE_STATIC_DEPENDENCIES)
        ...
        _qt5_UiTools_process_prl_file(
            "${_qt5UiTools_install_prefix}/lib/Qt5UiTools.prl" RELEASE
            _Qt5UiTools_STATIC_RELEASE_LIB_DEPENDENCIES
            _Qt5UiTools_STATIC_RELEASE_LINK_FLAGS
        )
    endif()
    ...
    set_property(TARGET Qt5::UiTools PROPERTY INTERFACE_QT_PLUGIN_TYPES "")
    ...
    _populate_UiTools_target_properties(RELEASE "Qt5UiTools.lib" "" FALSE)

    if (EXISTS "${_qt5UiTools_install_prefix}/lib/Qt5UiToolsd.lib" )
        _populate_UiTools_target_properties(DEBUG "Qt5UiToolsd.lib" "" FALSE)
    endif()
